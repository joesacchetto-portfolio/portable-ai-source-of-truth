...
**How to use:** Copy everything below the line into a session, along with your Context Seed (paste it or let the tool read the file). If the tool can write files directly, it creates the live files for you; otherwise it outputs each file's content for you to paste in yourself. If the assistant cannot read the repo files directly, paste the Context Seed and the relevant blank starter templates (found under each folder's `starter/` subfolder), or paste the structure you want it to follow.

---

I have a "Context Seed" document (provided) and a folder containing blank starter templates (each folder's `starter/` subfolder). Your job is to sort the seed's content into the right files. Rules:
