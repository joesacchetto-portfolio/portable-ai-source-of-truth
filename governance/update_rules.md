...
"Locked" is not a separate status — it's an Approved item living in `source_of_truth/locked_facts.md`, which gets the strictest review (see below).
...
- Update `core_context/current_working_context.md` to reflect what the human said in the current session — quoting, not interpreting — entering at DRAFT — Needs Review (see `approval_boundaries.md`, "Allowed without sign-off").
...
- Mark anything Approved, or move anything into `source_of_truth/locked_facts.md` — even if instructed to "approve it all." New or AI-generated material becomes Approved only after a human reads that specific item and confirms it. (One exception: inserting the status after the human confirms a specific item and gives an explicit item-level instruction — see step 4 below.)
